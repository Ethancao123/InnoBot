 ...
 *  rwheel : Tick counts from the right motor encoder (std_msgs/Int16)
 * 
 *  lwheel : Tick counts from the left motor encoder  (std_msgs/Int16)
 * 
...
const double TICKS_PER_REVOLUTION = 663;//1326 // For reference purposes.
const double WHEEL_RADIUS = 0.625; // Wheel radius in meters //this is wrong for sure
const double WHEEL_BASE = 0.272; // Center of left tire to center of right tire
const double TICKS_PER_METER = 3385; // Original was 2800 //needs to be updated //this is also wrong
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subForRightCounts = node.subscribe("rwheel", 100, Calc_Right, ros::TransportHints().tcpNoDelay());
  ros::Subscriber subForLeftCounts = node.subscribe("lwheel", 100, Calc_Left, ros::TransportHints().tcpNoDelay());
